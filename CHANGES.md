...
- skill-quality-guard-v1.1 (corrigendum) — spec: n/a (post-ship smoke audit corrigendum) | shipped: 2026-04-30 | source: internal smoke audit | quality-guard-version: v1 (audited against pinned prior; v1.1 changes are additive + narration-only)
  - workflow/references/quality-guard.md: EDIT-body | anchor: header version `v1` → `v1.1` + `## Pre-Fill Heuristic` (new CHANGES.md-only paragraph after table) + new `## Changelog` section at file end | extractable: yes | adds CHANGES.md-only auto-N/A relief + records v1.1 changelog
  - specs/shipped/2026-04-30-skill-quality-guard.md: EDIT-body | anchor: `## Quality Guard Results` table row QG-7 | extractable: n/a | narration corrected ("phrase match" → "detection match"); rule semantics unchanged
  - CHANGES.md: EDIT-body | anchor: this entry | extractable: n/a | self-reference (housekeeping)
  - Notes: Smoke audit run as read-only general-purpose agent against shipped v1. Two SUSPECT findings, both informational, neither blocking. Patches are additive (new pre-fill row) + narration-only (no Tier 1 / Tier 2 rule wording changed; no SKIP added; no router edit). Specs/shipped/ path is NOT a skill-improvement trigger (per Detection Rule), so the shipped-spec edit is exempt from Quality Guard self-application. The quality-guard.md edit IS skill-improvement; per QG-versioning rule, this v1.1 corrigendum audits against pinned v1. Self-application: QG-1 N/A (no external source), QG-2 PASS (no compression mode extended), QG-3 PASS (no triggers narrowed), QG-4 N/A (no router edits), QG-5 PASS (no SKIP added), QG-6 PASS (spec template untouched), QG-7 PASS (no new file references), QG-8 N/A (review spec untouched), QG-9 N/A (corrigendum, no spec / AC mapping required), QG-10 PASS (single-commit revert restores v1), QG-11..18 N/A (additive narration-only patch, no overlap, no overlay, neutral token cost, no analysis section, no triggers, README+SKILL.md untouched), QG-19/20 PASS (count tracked, self-application this row). Final verdict: GUARD-CLEAN.
